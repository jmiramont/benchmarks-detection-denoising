function [tfr, stfr, lost, q_hatmap, if_hatmap] = tfrvsgab2(x, M, L, q_method, if_method, gamma_K, q_threshold)
% [tfr, stfr, lost, q_hatmap, if_hatmap] = tfrvsgab(x, M, L, q_method, if_method, gamma_K, q_threshold)
% Compute the discrete-time vertical synchrosqueezed Gabor Transform (using FFT)
% 
% INPUT:
...
% Ref: [D. Fourer, F. Auger, K.Czarnecki, S. Meignen and P. Flandrin, Chirp rate and instantaneous frequency estimation. IEEE Signal Processing Letters 2017]
...
for n = 1:N
  ...
  tfr(:,n) = fft(x(n+k) .* g_Ts,    M) .* exp(B * mm * (n-1 - k_min));
  
  tfr_t      = fft(x(n+k) .* tg,      M) .* exp(B * mm * (n-1 - k_min));
  tfr_d      = fft(x(n+k) .* dg_Ts2,  M) .* exp(B * mm * (n-1 - k_min));
  tfr_td_v   = fft(x(n+k) .* tdg_Ts,  M) .* exp(B * mm * (n-1 - k_min));
  tfr_t2     = fft(x(n+k) .* t2g_Tsm1,M) .* exp(B * mm * (n-1 - k_min));
  tfr_d2     = fft(x(n+k) .* d2g_Ts3, M) .* exp(B * mm * (n-1 - k_min));
  %tfr_t2dg   = fft(x(n+k) .* t2dg,    M) .* exp(B * mm * (n-1 - k_min));
  tfr_t3     = fft(x(n+k) .* t3g_Tsm2,M) .* exp(B * mm * (n-1 - k_min));
  ...
  for m = 1:M
    ...
    if abs(tfr(m,n)) > eps
     ...
     tfr_t_Tsm1 = tfr_t(m);     %exp(B * mm(m) * nn) * sum( x(n+k) .* tg     .* exp(B .* mm(m) .* k));  % 
     tfr_d_Ts   = tfr_d(m);     %exp(B * mm(m) * nn) * sum( x(n+k) .* dg_Ts2 .* exp(B .* mm(m) .* k));  %exp(B * mm * nn) * 
     
     tfr_td      = tfr_td_v(m); %exp(B * mm(m) * nn) * sum( x(n+k) .* tdg_Ts   .* exp(B .* mm(m) .* k));  % 
     tfr_t2_Tsm2 = tfr_t2(m);   %exp(B * mm(m) * nn) * sum( x(n+k) .* t2g_Tsm1 .* exp(B .* mm(m) .* k));  %exp(B * mm * nn) * 
     tfr_d2_Ts2  = tfr_d2(m);   %exp(B * mm(m) * nn) * sum( x(n+k) .* d2g_Ts3  .* exp(B .* mm(m) .* k));  %exp(B * mm * nn) * 
     
     if q_method == 4
      tfr_t2d_Tsm1 = tfr_t2(m);   %exp(B * mm(m) * nn) * sum( x(n+k) .* t2dg     .* exp(B .* mm(m) .* k));
      tfr_t3_Tsm3  = tfr_t3(m);   %exp(B * mm(m) * nn) * sum( x(n+k) .* t3g_Tsm2 .* exp(B .* mm(m) .* k));
