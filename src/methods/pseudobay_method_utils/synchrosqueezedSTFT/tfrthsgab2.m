function [tfr, stfr, lost, lost2] = tfrthsgab2(x, M, L,gamma_K, q_method)
...
for n = 1:N
  ...
  for m = 1:M
    ...
    if abs(tfr(m,n)) > eps
     ...
     tfr_t2_Tsm2 = tfr_t2(m); %exp(B * mm(m) * nn) * sum( x(n+k) .* t2g_Tsm1 .* exp(B .* mm(m) .* k));  %exp(B * mm * nn) *   %%w2
     tfr_d2_Ts2  = tfr_d2(m); %exp(B * mm(m) * nn) * sum( x(n+k) .* d2g_Ts3  .* exp(B .* mm(m) .* k));  %exp(B * mm * nn) *   %%t2
