function signal_r = contour_filtering(s, NumC, Ks, a, Nfft)
N = length(s);
...
% figure(); 
% imagesc(basins);set(gca,'clim',[-0 4],'ydir','normal');
% hold on
% for i=1:NumC
%     plot(contours{i}.x,contours{i}.y,'r-','linewidth',2)
% end
% for i=NumC+1:length(contours)
%     plot(contours{i}.x,contours{i}.y,'k-','linewidth',2)
% end
% set(gca,'YDir','normal');
...
for i = 1:fM,
    for j = 1:fN,
        ...
        LL = 11;
        tmp = TempCon(max(1,k-LL):min(fM,k+LL),max(1,p-LL):min(fN,p+LL));
        basins(i,j) = mode(tmp(:));
    end
end
