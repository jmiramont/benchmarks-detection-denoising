function [tfr, rtfr, lost] = tfrlmgab2(x, M, L, mu, gamma_K)
% [tfr, rtfr, lost] = tfrlmrgab2(x, M, L, mu, gamma_K)
% Compute the discrete-time Levenberg-Marquard reassigned Gabor Transform
% (using FFT)
% 
% INPUT:
% x      : the signal to process
% M      : number of frequency bins to process (default: length(x))
% L      : window duration parameter:  L=T/Ts, (default: 10)
% mu     : damping parameter
% gamma_K: threshold applied on window values (default: 10^(-4))
%
% OUTPUT:
% tfr    : discrete Gabor spectrogram
% rtfr   : LM Reassigned Gabor spectrogram
%
% Author: D.Fourer
% Date: 28-08-2015
