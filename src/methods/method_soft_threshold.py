...
def soft_thresholding(signal, coeff=1, dict_output=False):
    Nfft = len(signal)
...
class NewMethod(MethodTemplate):
    def __init__(self):
        self.id = 'soft_thresholding'
        self.task = 'denoising'


    def method(self, signal, *args, **kwargs):
        signal_output = soft_thresholding(signal, *args, **kwargs)          
