...
/* Default values of time constants */
#define TAU0 0.005  
#define TAU1 0.05
#define TAU2 0.129
#define TAU3 0.253
...
void NrglInit (adl_struc *adl,float smplfrq, float LowpassCutoff,float *tau)
{
     ...
     short i, j ;
     float tauout;
     /* adl_struc adl[1]; wird nun in main definiert */
     
     /* brain lowpass at output */
     if (LowpassCutoff) {  /* skip if == 0 (no LP-filtering) */
        tauout = 0.5 / (M_PI * LowpassCutoff); 
        /* Calculate time constant from cut-off frequency */
    ...
    */
    ...
    /* return adl; wird nun als Parameter übergeben*/
...
/************************** NRGL *************************************
  ...
  Return:
         none
 ********************************************************************** */
void Nrgl(adl_struc *adl, double *data_in, double *data_out, int framelen, int q_out_lp, int qlimit) 
{
    float tmp ;
    int i, j ;
    ...
    for (i=0; i<framelen; i++) {     /* take every sample */     
        ...
        for (j=0; j<NSTAGE; j++) {
            ...
            /* low passfilter the divisor of stage j */
...
{
    mexPrintf("\n");
    mexPrintf(" Usage: OUT = ADAPT(IN, FSAMPLE, [LP_CUTOFF],[TAU1],[TAU2],[TAU3],[TAU4],[TAU5])\n");                                                           
    mexPrintf("\n");                                                                      
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    double *in, *out;
    int i, m, n, len;
    float fsample, LowpassCutoff = OUT_LOWPASS_CUTOFF;
    int qlimit = QLIMIT, q_out_lp = true;
    adl_struc *adl;
    int exit = false;
    float tau[5];
    ...
    if ((nrhs < 2) || (nrhs > 8) || (nlhs != 1) || (nrhs == 7)  || (nrhs == 6)|| (nrhs == 5)|| (nrhs == 4) ) {
        usage();
        exit = true;        /* abort */
    } 
    ...
    if (!exit) {
        ...
        /* actual computations */
        /* adl = NrglInit(fsample, LowpassCutoff);*/
        NrglInit(adl,fsample, LowpassCutoff, tau);
        Nrgl(adl, in, out, len, q_out_lp, qlimit); 
    }
